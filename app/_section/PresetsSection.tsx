...
export default function PresetsSection({ activePresetId, onApply }: { activePresetId: string | null; onApply: (preset: StudioPreset) => void }) {
  ...
  const families = useMemo(() => ["all", ...Array.from(new Set(GRID_PRESETS.map((preset) => preset.family)))], []);
  const sizes = useMemo(() => ["all", ...Array.from(new Set(GRID_PRESETS.map((preset) => preset.size)))], []);
  const filtered = GRID_PRESETS.filter((preset) => [preset.family, preset.archetype, preset.variant, preset.size, ...preset.tags].join(" ").toLowerCase().includes(query.toLowerCase()) && (family === "all" || preset.family === family) && (size === "all" || preset.size === size));
  const surprise = () => onApply((filtered.length ? filtered : GRID_PRESETS)[Math.floor(Math.random() * (filtered.length ? filtered : GRID_PRESETS).length)]);
